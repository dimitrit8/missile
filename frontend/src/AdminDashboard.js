...
import { Users, Globe, MapPin, Calendar, Lock, Eye, ChartLineUp } from "@phosphor-icons/react";
  ...
  const StatCard = ({ icon: Icon, label, value, color }) => (
    <div className="bg-[#141414] border border-[#27272A] rounded-lg p-4">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-sm text-[#A1A1AA] uppercase tracking-wider">{label}</span>
      </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-3">
            <button
              onClick={refreshData}
              className="px-4 py-2 bg-[#141414] border border-[#27272A] rounded-lg hover:bg-[#1C1C1E] transition-colors"
            >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <StatCard icon={Users} label="Total Visits" value={analytics?.total_visitors} color="text-[#007AFF]" />
          <StatCard icon={Globe} label="Unique Visitors" value={analytics?.unique_visitors} color="text-[#34C759]" />
          <StatCard icon={Calendar} label="Today" value={analytics?.today_visitors} color="text-[#FF9500]" />
          <StatCard icon={ChartLineUp} label="This Week" value={analytics?.week_visitors} color="text-[#FF3B30]" />
        </div>
